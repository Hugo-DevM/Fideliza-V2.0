...
// Business benefit icons — decorative, no translation needed
const BENEFIT_ICONS = [
              ...
              {t.items.map((benefit, i) => (
                <li key={i} className={`reveal reveal-d${i + 1} flex gap-4`}>
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500 leading-relaxed">{benefit.body}</div>
                  </div>
                </li>
              ))}
            </ul>
                ...
                <div>
                  ...
                  {t.dashboard.activity.map((item, i) => (
                    <div key={i} className="flex items-center gap-3 py-2 border-b border-gray-50 last:border-0">
                      <div className={`w-7 h-7 rounded-full ${AVATAR_COLORS[i]} flex items-center justify-center text-[11px] font-bold text-white flex-shrink-0`}>
                        {item.name[0]}
                      </div>
