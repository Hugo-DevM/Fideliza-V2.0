...
export default async function CustomerTransactionsPage({
  ...
  let initialRows: Awaited<ReturnType<typeof loadMoreCustomerTransactions>>['rows'];
  let hasMore: boolean;
  try {
    const db = createServiceRoleClient();
    ...
    ({ rows: initialRows, hasMore } = await loadMoreCustomerTransactions(id, 0));
  } catch (err) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{customerName}</p>
      </div>
