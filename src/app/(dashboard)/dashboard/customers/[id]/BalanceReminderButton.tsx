...
import { sendBalanceReminderAction } from './actions';
...
export default function BalanceReminderButton({
  ...
  function handleSend() {
    startTransition(async () => {
      const result = await sendBalanceReminderAction(customerId, programId);
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      >
        ...
        {isPending ? 'Enviando…' : status === 'sent' ? 'Enviado' : status === 'error' ? errorMsg : 'Recordar saldo'}
      </button>
    </div>
  );
}
