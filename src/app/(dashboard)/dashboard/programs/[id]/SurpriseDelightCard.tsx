...
export default function SurpriseDelightCard({
  programId,
  plan,
  programType,
  config,
}: SurpriseDelightCardProps) {
  ...
  const [probability, setProbability] = useState(
    Number(config.surprise_probability ?? 0.1),
  );
  const [multiplier, setMultiplier] = useState(
    isDiscrete && savedMultiplier === 1.5 ? 2 : savedMultiplier,
  );
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500">
              Puntos extra en visitas aleatorias
            </p>
          </div>
        </div>
      ...
      {!isPro ? (
        <div className="px-5 py-6 text-center">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Sorpresa Especial está disponible en el plan{" "}
        ...
        <div
          className={`space-y-4 px-5 py-4 ${!enabled ? "pointer-events-none opacity-40" : ""}`}
        >
          <p className="text-xs text-gray-400 dark:text-gray-500">
            En cada earn, hay una probabilidad de que el cliente reciba puntos
            extra como sorpresa. El cliente no sabe cuándo ocurrirá — eso es lo
          ...
          <div className="rounded-xl bg-violet-50 dark:bg-violet-500/10 px-4 py-3">
            <p className="text-xs text-violet-700 dark:text-violet-300">
              Con estas opciones,{" "}
              <strong>1 de cada {Math.round(1 / probability)} visitas</strong>{" "}
              dará <strong>{multiplier}× puntos</strong> como sorpresa.
            </p>
          </div>
        </div>
      )}
