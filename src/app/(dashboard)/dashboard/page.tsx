...
export default async function DashboardPage() {
  ...
  const stats = [
    ...
    { label: 'Programas activos',      value: programs.length,          href: '/dashboard/programs',  accent: 'text-purple-600' },
    { label: 'Transacciones hoy',      value: txTodayCount ?? 0,        href: '/dashboard/customers', accent: 'text-green-600' },
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-0.5 text-sm text-gray-500">
          Bienvenido de nuevo — <span className="font-medium">{tenant.name}</span>
        </p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2 lg:items-start">
        ...
        <div className="rounded-xl border bg-white shadow-sm">
          <div className="flex items-center justify-between border-b px-5 py-3">
            <h2 className="text-sm font-semibold text-gray-700">Programas</h2>
            <Link href="/dashboard/programs" className="text-xs text-indigo-600 hover:underline">Gestionar →</Link>
          </div>
          {!programs.length ? (
            ...
            <ul className="divide-y">
              {programs.slice(0, 5).map((p) => (
                <li key={p.id} className="flex items-center justify-between px-5 py-3">
                  ...
                  <span className="rounded-full bg-green-50 px-2 py-0.5 text-xs font-medium text-green-700">activo</span>
                </li>
              ))}
            </ul>
          )}
      ...
      <div className="rounded-xl border border-indigo-100 bg-indigo-50 p-4">
        <p className="text-sm font-semibold text-indigo-800">Portal de clientes</p>
